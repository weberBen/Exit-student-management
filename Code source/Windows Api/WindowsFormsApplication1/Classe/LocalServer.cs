...
namespace LocalServer
{
    ...
    class RequestTags
    {
       ...
       */
        ...
        */
        ...
        public const string CHANGE_SHORT_ID = "set_new_short_id";
        ...
        public string new_short_id { get; set; }
        ...
        public int inactive_state { get; set; }
                ...
                */
                ...
                switch (json_request.request_object)//oject of the request
                {
                    ...
                    case RequestTags.GET_LOG:
                        {
                            ...
                            response.AddParm(nameof(json_request.active_state), Definition.DO_NOT_DELETE_VALUE_DATABASE);
                            response.AddParm(nameof(json_request.inactive_state), Definition.DELETE_VALUE_DATABASE);
                            response.AddParm(nameof(json_request.student_scope), Definition.TYPE_OF_STUDENT_TABLE);
                            response.AddParm(nameof(json_request.division_scope), Definition.TYPE_OF_DIVISION_TABLE);
                            response.AddParm(nameof(json_request.current_date), Tools.dateTimeToString(DateTime.Now));

                            string res = database.retrieveAllInformationsByIndex(json_request.type_of_table, json_request.row_index);
                    ...
                    case RequestTags.CHANGE_PASSWORD:
                        ...
                        break; 
                    case RequestTags.CHANGE_SHORT_ID:
                        {
                            /* User want to change is password*/
                            CertifyAccreditationLevel = SecurityManager.RIGHTS_LIST.LastIndexOf(SecurityManager.RIGHT_SCHOOL_OFFICE_MEMBER);
                            ...
                            else//current id and password are correct, then change the password
                            {
                                ...
                                SecureString short_id = DataProtection.unprotect(short_secure_id);

                                if (database.changeAgentShortSecureId(input_agent.tableId, short_secure_id)!=Definition.NO_ERROR_INT_VALUE)
                                {
                                    process_error = true;
                                    message_to_display = "Une erreur est survenue :\nImpossible d'enregistrer les modifications";
                                }
                                else
                                {//notify the agent that his password changed (by mail)
                                    string header = "CHANGEMENT DE CODE D'IDENTIFICATION";
                                    string body = "Votre code d'identification vient d'être modifié. "
                                        + "Pour des raisons de sécurité nous ne communiquerons pas le nouveau code d'identification.<br>"
                                        + "Si vous n'êtes pas à l'origine de ce changement, veuillez en informer "
                                        + "l'administration au plus vite afin de régulariser la situation.";
                                    Tools.sendMail(header, body, new List<string> { database.getAgentMailAdress(input_agent.tableId) });
                                    //we don't use input_agent.mailAdress because that value is not update since the user has been authentificated

                                    response.AddParm(nameof(json_request.new_short_id), DataProtection.ToInsecureString(short_id));
                                }
    ...
    public class HttpServer : IDisposable
    {
        ...
        */
        ...
        private string[][] directories_list_ajax_request =
                {
        ...
        private string[] contentType_list_ajax_request =
                {
                "image/*",
                "audio/*",
            ...
            */
            ...
            {//client use ajax request
                ...
                */
                ...
                string path_to_file = Tools.getPathInPrivateOrPublicDirectoryFromFileName(directories_list_ajax_request[index], fileName_list[index]);
                ...
                else
                {
                    try
                    {
                        ...
                        content_type = contentType_list_ajax_request[index];
