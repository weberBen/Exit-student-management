...
namespace WindowsFormsApplication1.Forms
{
    public partial class AccessManagementForm : Form
    {
        ...
        private void save_change_button_Click(object sender, EventArgs e)
        {
            ...
            string subject;
            SecureString body = new SecureString();

            if (agent.tableId==-1)//if user add a new agent we automatically set a new short id for him
            {
                SecurityManager.setAgentShortSecureId(ref agent);
                res = database.addNewAgent(agent);

                subject = "Création de votre compte";

                foreach (char c in (agent.lastName.ToUpper()+" "+agent.firstName.ToLower()+" ("+agent.job.ToUpper()+") : "
                                    ...
                                    + " Cependant nous vous invitions à supprimer ce mail et changer votre mot de passe, si un mot de passe vous a été attribué"))
                {
                    body.AppendChar(c);
                }
                body.MakeReadOnly();
            ...
            }else
            {
                ...
                foreach (char c in (agent.lastName.ToUpper() + " " + agent.firstName.ToLower() + " (" + agent.job.ToUpper() + ") : "
                                    ...
                                    + "<br>Mot de passe : " + password_textBox.Text
                                    + "<br>Code d'identification : " + DataProtection.ToInsecureString(DataProtection.unprotect(agent.shortSecureId))
                                    + "<br><br>Selon votre niveau d'accréditation, vous n'avez pas besoin d'un identifiant et d'un mot de passe."
            ...
            if (res == ToolsClass.Definition.NO_ERROR_INT_VALUE)//no error
            {
                ...
                */
                ...
                if(res_mail!=ToolsClass.Definition.NO_ERROR_INT_VALUE)
                {
                    message += "\nAttention : le mail n'a pas pu être envoyé";
                }else
                {
