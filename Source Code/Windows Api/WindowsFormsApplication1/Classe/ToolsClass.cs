...
namespace ToolsClass
{
    static class Definition
    {
        ...
        public static Tools.ParmsDatabase ParmsDatabase = Settings.Database;
        ...
        public const int TYPE_OF_AUTHORIZATION_TABLE = 3;
        public const int TYPE_OF_BAN_TABLE = 4;
        ...
        public static int COMPUTER_TABLE_ID_DATABASE = -1;//the value is set in the main form at the start of the app
        /*The computer agent is a fake agent store into the database with the right -1, no id and no password*/
        public const string COMPUTER_AUTHENTIFICATION_ID_DATABASE = "Computer";
    ...
    class Tools
    {
        ...
        {
            string file_photo = "";
            string extension_photo = "";

            if (!string.IsNullOrEmpty(path_photo))//file exists
            {
                try
                {
                    byte[] bytes = File.ReadAllBytes(path_photo);
                    file_photo = Convert.ToBase64String(bytes);

                    extension_photo = Path.GetExtension(path_photo);
                    extension_photo = extension_photo.Replace(".", "");//extension = ".jpg", need to remove "."

                }
                catch { }
            }

            return Tuple.Create(file_photo, extension_photo);
        }
        ...
        public static string getFormatedStringForSessionInfo(string last_name, string first_name, string job)
        {
            ...
            return last_name + " " + first_name + " (" + job + ")";
        }

        public static string getFormatedStringAgentInfo(string last_name, string first_name, string job)
        {
            /*From the agent last name, first name and job return a string that gather all that information*/
            if (last_name == null)
                last_name = "";
                ...
                */
                switch (format)
                {
                    ...
                    case Definition.TEXT_FORMAT_NORMALIZE://normalized
                        ...
                        break;
                    case Definition.TEXT_FORMAT_INVISIBLE://current element must not be included into the format
                        element_value = null;
                        break;
                }
            };
    ...
    class Settings
    {
        ...
        */
        ...
        private const string XML_ATTRIBUTE_VALUE_PARMS_DATABASE = "Database";
        private const string XML_ATTRIBUTE_VALUE_SQL_SERVER_NAME = "Sql_server_domain_name";
        ...
        public static Tools.ParmsDatabase Database
        {
            get
            {
                List<string> attribute_element_list = new List<string>(new string[] {XML_ATTRIBUTE_VALUE_SQL_DRIVER_NAME,
                XML_ATTRIBUTE_VALUE_SQL_SERVER_NAME, XML_ATTRIBUTE_VALUE_SQL_DATABASE_NAME});
                ...
                List<string> elements = XmlHelper.readXmlListElement(Definition.PATH_TO_TOOLS_FILE, Definition.SECTION_TAG_XML_FILE, Definition.ATTRIBUTE_ELEMENT_XML_FILE,
                                              XML_ATTRIBUTE_VALUE_PARMS_DATABASE, Definition.XML_TAG_FOR_ITEM,
                                              Definition.ATTRIBUTE_ELEMENT_XML_FILE, attribute_element_list);


                Tools.ParmsDatabase res = new Tools.ParmsDatabase();
                res.toDefault();

                try
                {
                    res.DriverName = elements[0];
                    res.ServerName = elements[1];
                    res.DatabaseName = elements[2];
                }catch { }
            ...
            set
            {
                Definition.ParmsDatabase = value; //update value into the app


                List<string> attribute_element_list = new List<string>(new string[] {XML_ATTRIBUTE_VALUE_SQL_DRIVER_NAME,
                XML_ATTRIBUTE_VALUE_SQL_SERVER_NAME, XML_ATTRIBUTE_VALUE_SQL_DATABASE_NAME});

                List<string> element = new List<string>(new string[] {value.DriverName, value.ServerName, value.DatabaseName});
